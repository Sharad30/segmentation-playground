...
def run_hyperparameter_tuning(
    ...
    for i, (lr, batch_size) in enumerate(all_combinations):
        ...
        try:
            ...
            results, metrics = train_multiclass_segmentation(
                ...
                lr=lr,
                resume_wandb=False,
                log_lr_per_epoch=True  # Add parameter to enable per-epoch LR logging
            )
            ...
            if metrics is not None:
                # Safely extract box mAP
                if hasattr(metrics, 'box') and hasattr(metrics.box, 'map50'):
                    box_map50 = metrics.box.map50
                    # Convert numpy type to Python float if needed
                    if hasattr(np, 'floating') and isinstance(box_map50, np.floating):
                        box_map50 = float(box_map50)
                    elif hasattr(np, 'number') and isinstance(box_map50, np.number):
                        box_map50 = float(box_map50)
                
                # Safely extract mask mAP
                if hasattr(metrics, 'seg') and hasattr(metrics.seg, 'map50'):
                    mask_map50 = metrics.seg.map50
                    # Convert numpy type to Python float if needed
                    if hasattr(np, 'floating') and isinstance(mask_map50, np.floating):
                        mask_map50 = float(mask_map50)
                    elif hasattr(np, 'number') and isinstance(mask_map50, np.number):
                        mask_map50 = float(mask_map50)
    ...
    try:
        ...
        try:
            ...
            # Convert all_results to a more usable format
            ...
            for row in all_results:
                try:
                    ...
                    lr = float(row[1])
                    bs = int(row[2])
                    box_map = float(row[3]) if row[3] is not None else 0.0
                    mask_map = float(row[4]) if row[4] is not None else 0.0
                    train_time = float(row[5]) if row[5] is not None else 0.0
                    
                    # Skip entries with all zeros if there are some non-zero entries
                    if box_map == 0.0 and mask_map == 0.0 and train_time == 0.0:
                        # Check if we have some successful runs
                        if any(r[3] > 0 or r[4] > 0 or r[5] > 0 for r in all_results):
                            print(f"Skipping likely failed run: {run_name}")
                            continue
                    
                    clean_results.append([run_name, lr, bs, box_map, mask_map, train_time])
                except Exception as e:
                    print(f"Error processing result row {row}: {e}")
            
            # Convert to numpy arrays for easier manipulation
            if clean_results:
                result_array = np.array(clean_results)
                lrs = result_array[:, 1].astype(float)
                batch_sizes = result_array[:, 2].astype(int)
                box_maps = result_array[:, 3].astype(float)
                mask_maps = result_array[:, 4].astype(float)
                ...
                sorted_lrs = sorted(list(set(lrs)))
                lr_data = {lr: [] for lr in sorted_lrs}
                for i, lr_val in enumerate(lrs):
                    lr_data[lr_val].append(box_maps[i])
                    
                sorted_bs = sorted(list(set(batch_sizes)))
                bs_data = {bs: [] for bs in sorted_bs}
                for i, bs_val in enumerate(batch_sizes):
                    bs_data[bs_val].append(box_maps[i])
                
                # Box plots
                if any(len(lr_data[lr]) > 0 for lr in lr_data):
                ...
                if any(len(bs_data[bs]) > 0 for bs in bs_data):
                ...
                lr_data = {lr: [] for lr in sorted_lrs}
                for i, lr_val in enumerate(lrs):
                    lr_data[lr_val].append(mask_maps[i])
                    
                bs_data = {bs: [] for bs in sorted_bs}
                for i, bs_val in enumerate(batch_sizes):
                    bs_data[bs_val].append(mask_maps[i])
                ...
                if len(results_df) > 0:
                    best_box_idx = results_df["Box mAP50"].idxmax() if not results_df["Box mAP50"].isna().all() else 0
                    best_mask_idx = results_df["Mask mAP50"].idxmax() if not results_df["Mask mAP50"].isna().all() else 0
